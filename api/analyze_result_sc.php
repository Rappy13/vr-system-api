<?php
// analyze_result.php - 分析游戏结果并返回评价
...
try {
    ...
    // 获取参数
    $gameData = $input['data'][0]; // 游戏数据
    $id = $input['id'] ?? null; // 场景 ID
    $FireExtin_id = $input['FireExtin_id'] ?? 0; // 灭火器 ID
    $game_timer = $input['game_timer'] ?? 0; // 游戏时间
    $TimeLimit = $input['TimeLimit'] ?? 0; // 时间限制
...
function analyzeScenes0to4($id, $FireExtin_id, $game_timer, $pressData, $pressCount, $isSuccess, $outfire, $sp_wrong) {
    ...
    if ($count >= 6 && !$sp_wrong) {
        ...
        return [
            'rating' => $count >= 5 ? '70分的侥幸成功' : '60分的侥幸成功',
            'medal' => 'none',
            'score' => $count >= 5 ? 70 : 60
        ];
    }
...
function analyzeScenes5to8($id, $FireExtin_id, $game_timer, $pressData, $pressCount, $isSuccess, $sp_wrong) {
    ...
    if ($count >= 3 && !$sp_wrong) {
        ...
        return [
            'rating' => $count >= 2 ? '70分的侥幸成功' : '60分的侥幸成功',
            'medal' => 'none',
            'score' => $count >= 2 ? 70 : 60
        ];
    }
...
function getPerfectRating($id, $FireExtin_id, $game_timer) {
    ...
    if ($game_timer < $thresholds[0]) {
        ...
        return ['rating' => '银牌高手 成功', 'medal' => 'silver', 'score' => 90];
    } else {
        return ['rating' => '铜牌高手 成功', 'medal' => 'bronze', 'score' => 80];
    }
}
